...
普通 Codex/Markdown 输出可以按上下文自然排版；当用户明确说要发到飞书、Lark、群 post、飞书文档，或输出将通过 `lark-cli` 发布/复制到飞书时，必须启用下面的“飞书输出模式”。

### 飞书输出模式

飞书模式目标是清晰、稳、好扫读，不依赖 Markdown 有序列表或嵌套列表渲染。

硬性格式：
- 不要使用 Markdown ordered list，例如 `1.`、`2.`、`10.`。
- 不要在“重点差异”里使用嵌套 `-`、`*` 子弹列表。
- 条目标题使用 `【NN｜等级｜领域】标题`，编号固定两位，例如 `【01｜高｜导入】insert_timeout 接管 INSERT-like 任务超时`。
- 等级使用：`阻断`、`高`、`中`、`低`、`需验证`。
- 领域使用短词：`导入`、`MV`、`配置`、`变量`、`缓存`、`存储`、`客户端`、`协议`、`权限`、`查询`。
- 字段行使用两个全角空格 `　　` 缩进，并使用短字段名：`旧行为`、`新行为`、`触发`、`影响`、`处理`。
- 条目之间用纯文本分隔线 `────────────────`，不要使用 Markdown 分割线 `---`。
- 配置名、变量名、header、SQL 关键字可以用 inline code；不要把长 `key=value`、长路径或整句包成 code span，避免飞书里出现难看的灰色长块换行。
- 每个字段尽量一行说完；处理方式有多步时写成 `1）...；2）...；3）...。`，不要换成子弹列表。
- `例子` 只在真的有助于理解时报，字段名写 `例：`，不要每条都强行加。

```text
升级结论
版本：<base> -> <target>
判断：<可以灰度升级 / 不建议直升 / 存在阻断项>
范围：<是否提供真实配置；未提供时说明是通用源码差异>

重点差异
────────────────
【01｜高｜导入】<变化点名称>
　　旧行为：说明基准版本的行为；如果基准版本没有该能力，明确写“旧版本没有该路径/参数/语义”。
　　新行为：说明目标版本的新行为；写出默认值、参数名、开关名或用户可见入口。
　　触发：什么 SQL、配置、Schema、MV、导入、查询或运行场景会触发。
　　影响：可能出现的报错、结果变化、性能变化、兼容性风险或运维影响。
　　处理：按需给配置、SQL、升级前检查、灰度验证、回滚或规避方案。
