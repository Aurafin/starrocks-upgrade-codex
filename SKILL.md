...
- 顶部摘要用一个 `div` 展示版本、判断、范围、重点风险数。
- “重点差异”每条使用真正的卡片分格：标题 `【NN｜等级｜领域】标题`，下面用 `fields` 做两列：`旧行为/新行为`、`触发/影响`，`处理` 单独一行。
- 每张卡最多放 6 条重点差异；超过 6 条时拆成多张卡，避免飞书卡片过长。
...
- 不要在“重点差异”里使用嵌套 `-`、`*` 子弹列表。
- 使用 `【NN｜等级｜领域】标题` 和短字段名 `旧行为`、`新行为`、`触发`、`影响`、`处理`。
- 条目之间用纯文本分隔线 `────────────────`。
